...
export class Player {
  ...
  emitMovementEvents() {
    ...
    if (this.accessibility.announceMovement) {
      const moved =
        Math.abs(this.x - this.lastX) > 1 || Math.abs(this.y - this.lastY) > 1;
      if (moved) {
  ...
  setState(state) {
    ...
    this.isJumping =
      state.isJumping !== undefined ? state.isJumping : this.isJumping;
    this.isRunning =
      state.isRunning !== undefined ? state.isRunning : this.isRunning;
    this.isCrouching =
      state.isCrouching !== undefined ? state.isCrouching : this.isCrouching;
