...
export class PersistenceManager {
    ...
    async save(key, data, options = {}) {
        ...
        try {
            ...
            if (this.storageConfig.compression) {
                serializedData = await this.compressData(JSON.stringify(dataWithMetadata));
            } else {
            ...
            if (serializedData.length > this.storageConfig.maxSize) {
                this.logger.error('Data too large to save:', key, serializedData.length);
                return false;
            }
            ...
            this.logger.info('Data saved successfully', { key, size: serializedData.length });
            return true;
        } catch (error) {
    ...
    async load(key, options = {}) {
        ...
        try {
            ...
            if (this.storageConfig.compression) {
                dataWithMetadata = JSON.parse(await this.decompressData(serializedData));
            } else {
            ...
            if (schema) {
                ...
                if (!validatedData) {
                    this.logger.error('Data validation failed after migration for key:', key);
                    return null;
                }
    ...
    async savePendingChanges() {
        ...
        this.logger.info('Saving pending changes', { count: this.pendingChanges.size });
        ...
        for (const [fieldName, fieldConfig] of Object.entries(schema.fields)) {
            ...
            if (value === undefined) {
                ...
                if (fieldConfig.type === 'number' && typeof value !== 'number') {
                    this.logger.error('Invalid type for field:', fieldName, 'expected number, got', typeof value);
                    return null;
                }
                if (fieldConfig.type === 'string' && typeof value !== 'string') {
                    this.logger.error('Invalid type for field:', fieldName, 'expected string, got', typeof value);
                    return null;
                }
                if (fieldConfig.type === 'boolean' && typeof value !== 'boolean') {
                    this.logger.error('Invalid type for field:', fieldName, 'expected boolean, got', typeof value);
                    return null;
                }
                if (fieldConfig.type === 'array' && !Array.isArray(value)) {
                    this.logger.error('Invalid type for field:', fieldName, 'expected array, got', typeof value);
                    return null;
                }
                if (fieldConfig.type === 'object' && typeof value !== 'object' || Array.isArray(value)) {
                    this.logger.error('Invalid type for field:', fieldName, 'expected object, got', typeof value);
                    return null;
                }
    ...
    async migrateData(key, dataWithMetadata) {
        ...
        while (currentVersion !== targetVersion) {
            const nextVersion = this.getNextVersion(currentVersion, Object.keys(migrations));
            ...
            if (migration) {
                ...
                this.logger.info('Data migrated', { key, from: currentVersion, to: nextVersion });
            } else {
                break;
            }
        }
    ...
    getNextVersion(currentVersion, availableVersions) {
        const sortedVersions = availableVersions.sort((a, b) => a.localeCompare(b, undefined, { numeric: true }));
    ...
    async loadFromBackup(key) {
        const backupKeys = this.getAllKeys().filter(k => k.startsWith(`${key}_backup_`));
